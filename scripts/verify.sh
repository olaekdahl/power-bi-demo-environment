...
printf '  %-24s %s\n' "R"                  "$(jq -r '.verify.R // "unknown"' <<<"$json")"
printf '  %-24s %s\n' "R packages"         "$(jq -r '.verify.RPackages // "unknown"' <<<"$json")"
printf '  %-24s %s\n' "Databases"          "$(jq -r '.verify.Databases // "unknown"' <<<"$json")"
printf '  %-24s %s\n' "FactInternetSales"  "$(jq -r '.verify.FactInternetSalesRows // "unknown"' <<<"$json") rows"
printf '  %-24s %s\n' "Spatial DB (PL300Demo)" "$(jq -r '.verify.SpatialObjects // "unknown"' <<<"$json"), $(jq -r '.verify.SpatialCustomers // "?"' <<<"$json")"
printf '  %-24s %s\n' "Spatial validation"  "$(jq -r '.verify.SpatialPointInPolygon // "unknown"' <<<"$json")"
